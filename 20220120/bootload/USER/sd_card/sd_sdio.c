...
SD_Error SD_Init(void)
{
	...
	//PC8/9/10/11---SDIO D0/1/2/3  PC12--SDIO_CK  PD2---SDIO_CMD
  ...
  GPIO_InitStructure.GPIO_Pin =GPIO_Pin_8|GPIO_Pin_12;//GPIO_InitStructure.GPIO_Pin =GPIO_Pin_8|GPIO_Pin_9|GPIO_Pin_10|GPIO_Pin_11|GPIO_Pin_12; 	//PC8,9,10,11,12复用功能输出	
  ...
  NVIC_InitStructure.NVIC_IRQChannel = SDIO_IRQn;
	...
	NVIC_Init(&NVIC_InitStructure);	//根据指定的参数初始化VIC寄存器、
 	...
 	if(errorstatus==SD_OK)errorstatus=SD_SelectDeselect((u32)(SDCardInfo.RCA<<16));//选中SD卡   
   	if(errorstatus==SD_OK)errorstatus=SD_EnableWideBusOperation(SDIO_BusWide_1b);	//1位宽度
